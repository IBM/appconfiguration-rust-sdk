...
use appconfiguration::{
    AppConfigurationClientHttp, ConfigurationId, Error, NetworkError, ServiceAddress, TokenProvider,
};
...
fn main() {
    ...
    assert!(matches!(
        client.unwrap_err(),
        Error::NetworkError(NetworkError::ReqwestError(_))
    ));
    ...
    assert!(matches!(
        client.unwrap_err(),
        Error::NetworkError(NetworkError::ProtocolError)
    ));
}
